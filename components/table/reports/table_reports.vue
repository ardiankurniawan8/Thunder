      ...
      <template slot="items" slot-scope="props">
        <tr @click="rowClicked(props.item.id)" style="cursor:pointer">
          ...
          <template v-for="x in computedHeaders">
            ...
            <td v-if="x.text === 'city'">{{ props.item.kota }}</td>
            <td v-if="x.text === 'date'">{{ new Date(props.item.created_at).toISOString().split('T')[0] }}</td>
          </template>
          <!-- ---------------------------------------------------------------------------------------------------------------- -->
          <!-- END TO MODIFY -->
          <!-- ---------------------------------------------------------------------------------------------------------------- -->
...
<script>
  import QueryReport from '~/apollo/queries/query_report'
      ...
      return {
        table: {
          ...
          availableHeaders: [
            ...
            { text: 'city', align: 'left', value: 'city', sortable: true },
            { text: 'date', align: 'left', value: 'date', sortable: true }
          ]
          // -----------------------------------------------------------------------------------------------------------------------
          // END TO MODIFY
          // -----------------------------------------------------------------------------------------------------------------------
    ...
    methods: {
      fetch () {
        ...
        this.$apollo.query(
          ...
          // TO MODIFY
          // -----------------------------------------------------------------------------------------------------------------------
          vm.table.data = result.data.AllReport
          vm.table.data_count = result.data.AllReportCount
          // -----------------------------------------------------------------------------------------------------------------------
          // END TO MODIFY
          // -----------------------------------------------------------------------------------------------------------------------

      ...
      <template slot="items" slot-scope="props">
        <tr @click="rowClicked(props.item.id)" style="cursor:pointer">
          ...
          <template v-for="x in computedHeaders">
            <td v-if="x.text === 'thumbnail'">{{ props.item.link_url }}</td>
            <td v-if="x.text === 'linked to'">{{ props.item.image_url }}</td>
            <td v-if="x.text === 'delete'">
            </td>
          </template>
          <!-- ---------------------------------------------------------------------------------------------------------------- -->
          <!-- END TO MODIFY -->
...
<script>
  import QuerySlider from '~/apollo/queries/query_slider'
      ...
      return {
        table: {
          ...
          availableHeaders: [
            ...
            { text: 'linked to', align: 'left', value: 'linked_to', sortable: true },
            { text: 'delete', align: 'left', value: 'delete', sortable: false }
          ]
          // -----------------------------------------------------------------------------------------------------------------------
          // END TO MODIFY
          // -----------------------------------------------------------------------------------------------------------------------
    ...
    methods: {
      fetch () {
        ...
        this.$apollo.query(
          ...
          // TO MODIFY
          // -----------------------------------------------------------------------------------------------------------------------
          if (result.data.Slider === null) {
            vm.table.data = []
            vm.table.data_count = 0
          } else {
            vm.table.data = result.data.Slider
            vm.table.data_count = result.data.Slider.length
          }
          // -----------------------------------------------------------------------------------------------------------------------
          // END TO MODIFY
          // -----------------------------------------------------------------------------------------------------------------------
